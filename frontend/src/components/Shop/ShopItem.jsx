    ...
    <div>
      {popUp ?
        <PopUp active={popUp} setActive={setPopUp} className='md:px-20 px-10'>
          <ItemSelection lang={props.lang} item={props.item} possessed={props.possessed} selected={props.selected} profile={props.profile} fetchProfile={props.fetchProfile}/>
        </PopUp> : null
      }
      <div onClick={() => {setPopUp(true)}}>
        <Card className={`${props.selected ? 'border-gradient' : ""} flex flex-col p-4 justify-center items-center hover:cursor-pointer border-gradient-hover hover:from-white/40 hover:to-white/10 hover:mx-[-5px] hover:my-[-5px]`}>
			...
			{props.possessed ?
				<div className="bg-black border-2 rounded text-white px-2 font-semibold">{language.owned[props.lang]}</div>
