...
export function Stat({ labels, data, colors, duration }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("resize", () => {
      setSize(window.screen.width < 768 ? 150 : 200);
    });
    return () => {
  ...
  return (
    <Card
      className={`flex items-center justify-center h-32 md:h-52 ${loaded ? "opacity-100" : "opacity-0"} transition-all duration-[${duration}ms]`}
    >
      <Chart
        options={opt.options}
        series={opt.series}
