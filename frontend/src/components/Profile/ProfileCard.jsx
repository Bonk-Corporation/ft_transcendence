...
export function ProfileCard({ setProfile, setTriedLog, fetchProfile }) {
  ...
  function updateProfile() {
    ...
    if (!("citation" in data) && !("password" in data)) return;
  ...
  return (
    <>
      <Card
        className={`flex flex-col items-center justify-center p-6 h-full ${loaded ? "opacity-100" : "opacity-0"} transition-all duration-300`}
      >
        <div className="flex items-center">
          <div
            className={`relative rounded-full md:block hidden w-32 mr-4 aspect-square border-4 bg-[url(${profile ? profile.avatar : null})] bg-center bg-cover`}
          >
          ...
          <div className="flex flex-col">
            ...
            <Input
              ref={citation}
              className="my-2"
              placeholder={language.citation[lang]}
              maxlength="256"
            />
          </div>
        </div>
        <div className="mt-4 mb-2 flex flex-col w-full items-center">
          <Input
            ...
            placeholder={language.new_password[lang]}
            type="password"
          />
          <Input
            ref={confirmPassword}
            className="w-full mt-2"
            placeholder={language.confirm_password[lang]}
            type="password"
          />
          <p className="mt-1 text-sm text-red-500">{error}</p>
        </div>
        <div className="flex mb-4">
          <CTA onClick={updateProfile} className="mr-2">
            {language.update[lang]}
          </CTA>
          <CTA
            onClick={logOut}
            className="ml-2 mr-2 border-red-500 border-2 text-red-500 hover:bg-red-500 hover:text-white"
            transparent={true}
          >
            {language.log_out[lang]}
          </CTA>
          <CTA
            onClick={deleteAccount}
            className="ml-2 border-red-500 border-2 text-red-500 hover:bg-red-500 hover:text-white"
            transparent={true}
          >
