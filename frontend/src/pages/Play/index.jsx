...
export function Play() {
  ...
  useEffect(() => {
    fetch("/api/citation").then((res) =>
      res.json().then((data) => {
        if (!data.error) setCitation(data);
      }),
    );
  ...
  return (
    <div className="flex flex-col h-full items-center justify-self-center">
      {citation ? (
        <h1 className="text-lg mb-4 text-center max-w-[1000px]">
          ❝{citation.citation}❞ - {citation.username}
        </h1>
      ) : null}
      <div className="w-screen max-w-[1000px] h-96 flex">
        <a
          id="anchor-pong"
          href="/pong"
          className="w-1/2 flex flex-col items-center rounded h-full bg-red-500 mx-4 hover:w-3/4 transition-all overflow-hidden group"
        >
