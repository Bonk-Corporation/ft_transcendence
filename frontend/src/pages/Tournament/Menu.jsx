...
export function Menu(props) {
  ...
  useEffect(() => {
    fetch(`/api/tournament/get_all_tournaments`).then((res) =>
      res.json().then((data) => {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="mb-4">
          <button
            onClick={() => setNbPlayers(2)}
            className={`mx-1 aspect-square px-2 rounded-full border-2 border-white font-semibold ${nbPlayers == 2 ? "bg-white text-black" : ""}`}
          >
            2
          </button>
          <button
            onClick={() => setNbPlayers(4)}
            className={`mx-1 aspect-square px-2 rounded-full border-2 border-white font-semibold ${nbPlayers == 4 ? "bg-white text-black" : ""}`}
          >
            4
          </button>
          <button
            onClick={() => setNbPlayers(8)}
            className={`mx-1 aspect-square px-2 rounded-full border-2 border-white font-semibold ${nbPlayers == 8 ? "bg-white text-black" : ""}`}
          >
            8
          </button>
        </div>
        <CTA onClick={createTournament} className="px-20">
          Create
        </CTA>
      ...
      <div className="flex items-center">
        <div className="flex flex-col items-center mr-4">
          ...
          <CTA
            ...
            className="w-72 hover:w-96 h-96 px-2"
          >
